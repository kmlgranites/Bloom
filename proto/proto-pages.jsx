...
function QuickAction({ title, onClick, active, danger, children }) {
  const [hover, setHover] = React.useState(false);
  const base = {
    width: 26, height: 26, borderRadius: 8,
    display:"inline-flex", alignItems:"center", justifyContent:"center",
    background: active ? gT.flame : (hover ? (danger ? "rgba(247,102,56,0.12)" : "rgba(2,47,54,0.07)") : "transparent"),
    color: active ? "#fff" : (danger && hover ? gT.flame : gT.muted),
    ...
    <div style={{flex: 1, display:"flex", flexDirection:"column", overflow:"hidden"}}>
      ...
      <div style={{padding:"0 28px 10px", flexShrink: 0}}>
        <div style={{
          ...
          <GM size={32} cap={gT.cyan} mood="happy"/>
      ...
      <div style={{flex: 1, overflowX:"auto", overflowY:"hidden", padding:"0 28px 24px"}}>
        <div style={{
          display:"grid", gridTemplateColumns:"repeat(5, minmax(224px, 1fr))",
          gap: 12, height:"100%", minWidth: 1180,
        }}>
          {STAGES.map(st => (
            <KanbanColumn
              key={st.id}
      ...
      {/* Job detail modal */}
      {s.params.detailJobId && <JobDetailModal/>}
    </div>
  );
}
...
function KanbanColumn({ stage, count, ghost, jobs, selected, onSelect, onSelectAll, selectMode }) {
  ...
  return (
    <div style={{
      background: stage.tint, borderRadius: 18, padding: 10,
      display:"flex", flexDirection:"column", minHeight: 0,
      border: `1px solid ${gT.hairline}`,
    }}>
      {/* Column header */}
      <div style={{padding:"6px 8px 12px", display:"flex", alignItems:"center", gap: 10, flexShrink: 0}}>
        <div style={{
          display:"inline-flex", alignItems:"center", justifyContent:"center",
      ...
      {/* Card list */}
      <div style={{flex: 1, overflowY:"auto", display:"flex", flexDirection:"column", gap: 8, padding:"2px 4px 4px"}}>
        {jobs.length === 0 ? (
    ...
    >
      ...
      <div style={{display:"flex", gap: 10, alignItems:"flex-start"}}>
        ...
        <div style={{flex:1, minWidth: 0, paddingTop: 1}}>
          <div style={{fontSize: 13.5, fontWeight: 700, color: gT.ink, lineHeight: 1.25, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis"}}>{job.role}</div>
      ...
      {/* Footer: meta on the left · quick actions on the right (reveal on hover) */}
      <div style={{display:"flex", alignItems:"center", gap: 4, minHeight: 26, marginTop: -2}}>
        <span style={{fontSize: 10.5, color: gT.muted, fontWeight: 600, flex: 1, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis"}}>
          {stage.id === "saved"
            ? job.loc
            : (stage.id === "offer" ? job.replyText
              : (job.appliedAt ? `Sent ${job.appliedAt}` : job.loc))}
        </span>

        {/* Quick actions — view detail (modal), open posting, move (menu), delete */}
        <div style={{display:"flex", gap: 2, alignItems:"center", opacity: showActions || moveOpen ? 1 : 0, transition:"opacity 0.12s", pointerEvents: (showActions || moveOpen) ? "auto" : "none"}}>
          <QuickAction title="View job details" onClick={()=>d({type:"OPEN_DETAIL", jobId: job.id})}><IcoEye/></QuickAction>
          <QuickAction title="Open job posting"><IcoOut/></QuickAction>
          <div style={{position:"relative"}}>
            <QuickAction title="Move / update status" onClick={()=>setMoveOpen(o=>!o)} active={moveOpen}><IcoMove/></QuickAction>
            {moveOpen && (
              <MoveMenu job={job} stage={stage} onClose={()=>setMoveOpen(false)} onMove={(st)=>{ d({type:"MOVE_STAGE", jobId: job.id, stage: st}); setMoveOpen(false); }}/>
            )}
          </div>
...
// ─────────────────────────────────────────────────────────────────────
function MoveMenu({ job, stage, onClose, onMove }) {
  ...
  return (
    <div ref={ref} onClick={(e)=>e.stopPropagation()} style={{
      ...
      width: 184, background:"#fff", borderRadius: 12,
